...
QuSeN_QuantumAcousticSensitivity::QuSeN_QuantumAcousticSensitivity(G4String name) :
  G4CMPElectrodeSensitivity(name), fileName("") {
  SetOutputFile(QuSeN_QuantumAcousticConfigManager::GetHitOutput());
}

/* Move is disabled for now because old versions of GCC can't move ofstream
QuSeN_QuantumAcousticSensitivity::QuSeN_QuantumAcousticSensitivity(QuSeN_QuantumAcousticSensitivity&& in) :
  G4CMPElectrodeSensitivity(std::move(in)),
  output(std::move(in.output)),
  fileName(std::move(in.fileName)) {
}

QuSeN_QuantumAcousticSensitivity& QuSeN_QuantumAcousticSensitivity::operator=(QuSeN_QuantumAcousticSensitivity&& in) {
...
QuSeN_QuantumAcousticSensitivity::~QuSeN_QuantumAcousticSensitivity() {
...
void QuSeN_QuantumAcousticSensitivity::EndOfEvent(G4HCofThisEvent* HCE) {
...
void QuSeN_QuantumAcousticSensitivity::SetOutputFile(const G4String &fn) {
  if (fileName != fn) {
    if (output.is_open()) output.close();
    fileName = fn;
    ...
    if (!output.good()) {
      ...
      G4Exception("QuSeN_QuantumAcousticSensitivity::SetOutputFile", "PhonSense003",
                  FatalException, msg);
      output.close();
    } else {
...
G4bool QuSeN_QuantumAcousticSensitivity::IsHit(const G4Step* step,
